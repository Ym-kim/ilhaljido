...
// ─────────────────────────────────────────────────────────────────────────────
// /moments 뷰 — app/moments/page.tsx 인라인 구현에서 추출 (2026-08-04 i18n-routes-v1)
// forceLang: /en·/ja 로케일 라우트용. 데이터·UI 카피는 원래 3언어 완비였음.
// ─────────────────────────────────────────────────────────────────────────────

function MomentCard({ moment, lang }: { moment: (typeof MOMENTS)[number]; lang: Lang }) {
  return (
    <Link
      href={localizeHref(`/select/hotel#${moment.anchor}`, lang)}
    ...
    <div className="min-h-screen bg-[#111] dark-surface">
      ...
      <section className="max-w-5xl mx-auto px-6 pb-14">
        <div className="flex flex-wrap items-center gap-2.5">
          <Link href={localizeHref('/guide', lang)} className="text-white/50 text-xs font-bold mr-1 hover:text-teal-300 transition-colors underline-offset-2 hover:underline">
            {ui.guidesLabel[lang]}
          </Link>
          {CITY_GUIDES.map((g) => (
            <Link
      ...
      {/* Submit CTA — 제보 폼은 KO 단일 라우트 (운영자 검수 KO) */}
