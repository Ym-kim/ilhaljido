...
export function Logo({ className, variant = 'dark' }: LogoProps) {
  ...
  const highlightId = `logo-hl-${uid}`
  const sunId = `logo-sun-${uid}`
  ...
  return (
    <Link
      href="/"
      className={cn('flex items-center gap-2.5 sm:gap-3 shrink-0 group rounded-xl focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sky-400', className)}
    >
      <svg
        width="36"
        height="36"
        ...
        className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 group-hover:scale-105 group-hover:rotate-[-3deg] transition-transform duration-300"
        aria-hidden
      >
        <rect width="36" height="36" rx="11" fill={`url(#${gradientId})`} />
        {/* 좌상단 하이라이트 — 유리 질감 깊이 */}
        <rect width="36" height="36" rx="11" fill={`url(#${highlightId})`} />
        ...
        <circle cx="27.2" cy="9.8" r="2.7" fill={`url(#${sunId})`} />
        ...
        />
        <defs>
          <linearGradient id={gradientId} x1="4" y1="2" x2="34" y2="36" gradientUnits="userSpaceOnUse">
            <stop stopColor="#7dd3fc" />
            <stop offset="0.35" stopColor="#0ea5e9" />
            <stop offset="0.72" stopColor="#0369a1" />
            <stop offset="1" stopColor="#0c4a6e" />
          </linearGradient>
          <radialGradient id={highlightId} cx="0.28" cy="0.14" r="0.9">
