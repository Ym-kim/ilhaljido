...
const checks = [
  ...
  ['Agoda falls back only when no valid property result remains', !files.adapter.includes('hotel !== null && Boolean(hotel.imageUrl)') && files.adapter.includes('.filter((hotel): hotel is StaySearchResult => hotel !== null)') && files.adapter.includes("reason: 'empty_result'") && files.engine.includes("mode: 'fallback'")],
  ...
  ['Landing URL CID, hotel, dates, guests and currency are validated', files.adapter.includes('safeAgodaLandingUrl') && files.adapter.includes('AGODA_STAY_PROVIDER.publicTrackingId') && ['hid', 'currency', 'checkin', 'checkout', 'NumberofAdults', 'NumberofChildren', 'Rooms'].every((key) => files.adapter.includes(key))],
  ...
  ['Missing provider photos use a neutral non-property placeholder', files.view.includes("'neutral_placeholder'") && files.view.includes('WAKATION STAY') && files.view.includes('숙소 사진은 Agoda에서 확인') && !files.view.includes('destinationImages[index % destinationImages.length]')],
  ['Provider property rating and guest review score use explicit sources and scales while zero values stay hidden', files.view.includes('formatProviderPropertyRating') && files.view.includes('{c.review} {guestScore.toFixed(1)}/10') && files.view.includes('result.starRating > 0') && files.view.includes('result.reviewScore > 0') && files.view.includes('{c.propertyClass} {formatProviderPropertyRating(propertyClass)}') && files.view.includes("propertyClass: 'Agoda 숙소 등급'") && files.view.includes("review: '숙박객 평점'") && files.view.includes('국가별 공식 호텔 등급과 다를 수 있습니다')],
  ...
  ['Exact pilot events are emitted', ['stay_search', 'stay_result_view', 'stay_property_click', 'stay_booking_click', 'affiliate_redirect'].every((event) => files.view.includes(`'${event}'`))],
  ...
  ['Discounts render only when provider fields are present and consistent', files.view.includes('getSafeDiscountPercentage') && files.view.includes('Math.abs(calculated - provided) <= 5') && files.view.includes('result.rate.crossedOutAmount > result.rate.amount')],
  ['Destination rollout status can disable Agoda without deleting Booking fallback', files.destinations.includes("'AGODA_READY' | 'BOOKING_FALLBACK' | 'DISABLED'") && files.adapter.includes("destination.rolloutStatus !== 'AGODA_READY'")],
  ['Result telemetry distinguishes image, discount and Wakation note presence', files.view.includes('imageStatus:') && files.view.includes('discountPresent:') && files.view.includes('wakationNotePresent:')],
  ...
  ['Result refinement preserves Wakation recommendation order by default', files.refinement.includes("sort === 'recommended'") && files.refinementBar.includes('Recommended')],
