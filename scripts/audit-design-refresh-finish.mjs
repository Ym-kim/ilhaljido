...
const source = {
  ...
  city: read('src/components/destinations/CityInsightView.tsx'),
...
const checks = [
  ...
  ['Expired August schedule copy and removed AI/Sokcho concept are absent', !/AI 크리에이터|AI Creator|AIクリエイター|강원 속초|Sokcho, Gangwon|江原・束草|8월[^'\n]{0,24}예정|August[^'\n]{0,24}(?:planned|TBD)|8月[^'\n]{0,24}予定/.test(source.data)],
  ['Stay card dates cannot be mistaken for a review score', [source.card, source.city].every((value) => !value.includes('8.6 기준')) && [source.card, source.city].every((value) => ['확인일', 'checked', '確認日'].every((label) => value.includes(label)))],
