...
const checks = [
  ...
  ['Pilot uses exact requested funnel events', files.pilotView.includes("trackStayEvent('stay_result_view'") && files.pilotView.includes("trackStayEvent('stay_property_click'") && files.pilotView.includes("trackStayEvent('affiliate_redirect'")],
  ['Pilot telemetry uses only bounded image and commerce flags', files.analytics.includes('image_status:') && files.analytics.includes('discount_present:') && files.analytics.includes('wakation_note_present:')],
